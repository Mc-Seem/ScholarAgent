...
- `219` backend tests pass, including schema-v3 integrity, strict defining-equation identity, ontology/qualifier separation, scoped notation, deterministic occurrences/injection, sparse projection, corpus metrics, semantic APIs, progress, and retrieval regressions.
- `476` frontend tests pass, including the unified term/formula lens, click/keyboard-only annotations, Equation Lens selection, sidebar/back/split behavior, glossary, schema-v3 graph details, workspace caches, shared Theia selections, and the user-facing `Term Highlights` vocabulary.
- `ScholarSemanticLensWidget.test.tsx` and the `Semantic Lens placement` suite in `ScholarContribution.test.ts` cover the side-view lens: right-area docking ahead of the authoring views, reveal without activation, sticky content on unrelated selections, stale-response and error handling, reader wording shown in place of the agent text with `Show original`, an edited symbol meaning saved against its notation subject, restore delegated to the subject-keyed delete, a paragraph comment that must not become the equation name, and an applied AI tooltip that must not become `edited` when a rebuild changes the graph wording.
- `TestSemanticNotes` in `tests/test_api.py` pins the note contract: the row is anchored to the subject rather than a DOM node, carries `is_user_override=true`, a second edit replaces the first, blank content is rejected, and restoring keeps `span.kg-entity` anchors in the stored HTML. Applying term highlights asserts the inverse provenance (`is_user_override=false`).
...
- Word-form coverage is pinned at both layers: `test_knowledge_graph_benchmark.py` reports exact versus conservative singular/plural and orthographic candidates, including an ambiguous-form bucket, while `test_knowledge_graph_canonical.py` accepts only singular/plural variants owned by one subject and rejects `models` when two semantic subjects claim `model`. A dry re-anchor of `arXiv:2602.02383` adds exactly the five forms predicted by the audit (`291 → 296` prose occurrences) without writing to the database.
- Tooltip-flow cleanup is contractual: `test_tooltip_suggestion.py` checks that the expertise prompt demonstrates schema-v3 `entity:`/`notation:` IDs, and `test_api.py` checks that `Paper` has no write-only `tooltip_suggestions_cache` column. Alembic revision `007` is the single migration head.
...
- `EquationLens.test.tsx` pins horizontal-only overflow for long notation symbols, while the Semantic Lens chrome tests require a lone hidden edit action to be out of flow and excluded from formula-title badge styling, so it neither reserves nor covers title text before hover.
...
- `SemanticDetails.test.tsx` pins the reader-facing wording and chrome of the lens: an `artifact` subject renders its own name with no taxonomy word anywhere in the panel, the location list names the section without repeating that word once per occurrence, and role tags carry `semantic-chip` rather than a Tailwind fill. Sibling cases read `styles/reader-interactions.css` directly, because contrast and hover rules are invisible to jsdom rendering: chips must be transparent with a border, the edit action must start at `opacity: 0`, be revealed on hover or keyboard focus, stay outside layout flow when it is the sole action, and never inherit the formula-title pill background; a disabled action must not dim itself with `opacity`, which would make a hidden button reappear.
- `TestSemanticEndpoints` verifies that subject evidence is projected in compiled paper order even when `evidence_ids` deliberately place an Introduction observation before Abstract.
